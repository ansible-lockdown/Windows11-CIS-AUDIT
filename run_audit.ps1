...
<#
.SYNOPSIS
    Run the CIS Windows 11 audit with syver.
...
# The version this audit was validated against. Everything it uses -- registry:,
...
$snapshotPath    = Join-Path $env:SystemRoot 'Temp\win11cis_policy_snapshot.txt'
$snapshotDir     = Join-Path $env:SystemRoot 'Temp\win11cis_snapshot'
...
function Get-MachineSid {
    $sid = Get-CimInstance Win32_UserAccount -Filter 'LocalAccount=True' -ErrorAction SilentlyContinue |
...
function New-PolicySnapshot {
    ...
    $lines.Add('Snapshot.Areas=SECURITYPOLICY,USER_RIGHTS,AUDITPOL,SERVICES,FIREWALL,HKU')
    ...
    $inf = Join-Path $env:TEMP "win11cis_secedit_$PID.inf"
    ...
    foreach ($row in @(& auditpol /get /category:* /r 2>$null)) {
        ...
        $guidByName[$name] = $fields[3].Trim()
        if ($inclusionToValue.ContainsKey($inclusion)) {
            $auditByName[$name] = $inclusionToValue[$inclusion]
        } else {
            $auditByName[$name] = "UNKNOWN=$inclusion"
        }
    }
    foreach ($target in $targets.auditpol) {
        if ($auditByName.ContainsKey($target.subcategory)) {
...
$auditJsonVars = @{
    ...
    win11cis_policy_snapshot = $snapshotPath
    win11cis_snapshot_dir    = $snapshotDir
